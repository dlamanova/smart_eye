...
class _DevicesScreenState extends State<DevicesScreen>
  ...
  Future<void> _loadInitialData() async {
    try {
      ...
      final rawServers = await _apiService.getServers().timeout(
    ...
    await Future.wait(
      servers.map((server) async {
        try {
          ...
          final cameras = cameraData
              .map((data) => Device.fromMap(data))
              .toList();
  ...
  List<Server> _mapToServers(List<Map<String, dynamic>> rawData) {
    return rawData.map((data) {
      ...
      final List<int> ipList =
          (data['ip'] as List<dynamic>?)?.cast<int>() ?? [0, 0, 0, 0];
                        ...
                        StreamBuilder<List<NotificationItem>>(
                          ...
                          builder: (context, notifSnapshot) {
                            final hasUnread =
                                ...
                                notifSnapshot.data!.any((n) => !n.isRead);
                            return IconButton(
                              onPressed: () {
...
class _ServerCard extends StatefulWidget {
  ...
  const _ServerCard({Key? key, required this.server, required this.onAddDevice})
    : super(key: key);
...
class _DeviceItem extends StatelessWidget {
  ...
  const _DeviceItem({Key? key, required this.device, required this.serverId})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () {
        debugPrint(
          '_DeviceItem: Navigating to monitoring with serverId="$serverId", deviceId="${device.uuid}"',
        );
        if (serverId.isEmpty) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Error: Server ID is missing. Cannot connect to stream.',
              ),
            ),
          );
          return;
